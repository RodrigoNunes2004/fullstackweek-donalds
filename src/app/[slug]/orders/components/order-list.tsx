...
const OrderList = ({ orders }: OrderListProps) => {
  ...
  // Auto-refresh the page every 3 seconds to check for status updates
  useEffect(() => {
        ...
        <Card>
          <CardContent className="p-5">
            <p className="text-sm text-muted-foreground">
              No orders found. Make sure you entered the correct name.
            </p>
          </CardContent>
        </Card>
      ) : (
            ...
            <CardContent className="space-y-4 p-5">
              <div
                className={`w-fit rounded-full px-2 py-1 text-xs font-semibold ${
                  order.status === OrderStatus.PAYMENT_CONFIRMED || order.status === OrderStatus.FINISHED
